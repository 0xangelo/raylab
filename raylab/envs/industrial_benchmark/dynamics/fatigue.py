...
import torch
from torch.distributions import Exponential, Uniform, RelaxedBernoulli
...
def sample_noise_variables(eff_velocity, eff_gain):
    ...
    eta_e_dist = Exponential(torch.empty_like(eff_gain).fill_(0.1))
    ...
    eta_u_dist = Uniform(torch.zeros_like(eff_gain), torch.ones_like(eff_gain))
    eta_gu = eta_u_dist.rsample()
    eta_vu = eta_u_dist.rsample()

    eta_gb = RelaxedBernoulli(2.0, torch.clamp(eff_gain, 0.001, 0.999)).rsample()
    eta_vb = RelaxedBernoulli(2.0, torch.clamp(eff_velocity, 0.001, 0.999)).rsample()
